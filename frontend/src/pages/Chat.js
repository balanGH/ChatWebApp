...
const Chat = () => {
  ...
  useEffect(() => {
    if (user && opUserData && opUserData.userId) {
      ...
      const fetchMessages = async () => {
        ...
        try {
          const response = await fetch(`http://10.11.145.24:5000/api/chat/${user.id}/${opUserData.userId}`);
          if (!response.ok) {
      ...
      // Fetch messages immediately when user or opUserData changes
      fetchMessages();
  ...
  }, [user, opUserData]);  // Depend on user.id and opUserData  
  ...
  const handleSendMessage = async () => {
    if (messageText.trim() && !sending) {
      ...
      try {
        const response = await fetch('http://10.11.145.24:5000/api/chat/send', {
          method: 'POST',
          headers: {
