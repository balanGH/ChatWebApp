...
function GroupDetails() {
  ...
  useEffect(() => {
    const fetchGroupDetails = async () => {
      try {
        const response = await fetch(`http://10.11.145.24:5000/group/${groupId}`);
  ...
  useEffect(() => {
    const checkAdminStatus = async () => {
      ...
      try {
        const response = await fetch(`http://10.11.145.24:5000/group/${groupId}/isAdmin`, {
          method: 'GET',
          headers: {
  ...
  const addMember = async () => {
    if (email) {
      try {
        const response = await fetch(`http://10.11.145.24:5000/group/${groupId}/members`, {
          method: 'POST',
  ...
  const removeMember = async (memberId) => {
    try {
      const response = await fetch(`http://10.11.145.24:5000/group/${groupId}/members/${memberId}`, {
        method: 'DELETE',
      });
  ...
  const toggleVisibility = async () => {
    ...
    try {
      const response = await fetch(`http://10.11.145.24:5000/group/${groupId}/visibility`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'user-id': userId,
  ...
  const toggleAdminOnlyMessages = async () => {
    ...
    try {
      const response = await fetch(`http://10.11.145.24:5000/group/${groupId}/adminOnlyMessages`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'user-id': userId,
