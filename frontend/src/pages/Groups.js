...
const Groups = () => {
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      ...
      try {
        const response = await fetch(`http://10.11.145.24:5000/api/getGroups/${userData.id}`); // Correct template literal usage
...
const handleCreate = async () => {
  ...
  try {
      const response = await fetch('http://10.11.145.24:5000/create_group', {
          method: 'POST',
          headers: {
...
const handleJoin = async () => {
  ...
  // Send data to the backend
  try {
      const response = await fetch('http://10.11.145.24:5000/join_group', {
          method: 'POST',
          headers: {
              'Content-Type': 'application/json',
          },
