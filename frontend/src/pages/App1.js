...
const Home = ({ isAuthenticated }) => {
  ...
  const fetchUserFnds = async () => {
    ...
    if (userData && userData.id) {
      try {
        const response = await fetch(`http://10.11.145.24:5000/api/getUserFnds/${userData.id}`, {
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    const fetchChatUsers = async () => {
      if (searchTerm) {
        try {
          const response = await fetch(`http://10.11.145.24:5000/chat-users?search=${searchTerm}`);
          const data = await response.json();
